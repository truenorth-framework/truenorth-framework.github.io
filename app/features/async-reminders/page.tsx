...
// This single file covers 3 feature pages:
//   /features/offline-first   → export default OfflineFirstPage
//   /features/multilingual    → export default MultilingualPage
//   /features/async-reminders → export default AsyncRemindersPage
//
// Drop the relevant export into each route's page.tsx file.
...
// ─────────────────────────────────────────────────────────────────────────────
export function OfflineFirstPage() {
  const ACCENT = "#4D9EFF";
  const POINTS = [
    {
...
// ─────────────────────────────────────────────────────────────────────────────
// PAGE 3 — Async Reminders
// ─────────────────────────────────────────────────────────────────────────────
export function AsyncRemindersPage() {
  const ACCENT = "#FFD700";
  const FLOW_STEPS = [
    {
      n: "1",
      title: "Session left incomplete",
      desc: "A user starts a conversation but doesn't complete all required fields. TrueNorth marks the session as pending.",
    },
    {
      n: "2",
      title: "Scheduler checks pending sessions",
      desc: "At your configured interval (hourly, daily, custom cron), TrueNorth queries for sessions that passed the reminder threshold.",
    },
    {
      n: "3",
      title: "LLM drafts a personalised message",
      desc: "The full session context is passed to the LLM with a persona prompt. It writes a specific, human-sounding follow-up — not a template.",
    },
    {
      n: "4",
      title: "Sent via WhatsApp, SMS, or custom webhook",
      desc: "The reminder is delivered through whichever channel the session originated on. WhatsApp Business API is supported out of the box.",
    },
  ];
  return (
    <>
      ...
      <PageHeader
        eyebrow="Async Reminders"
        title="The only framework that initiates contact."
        subtitle="Configure follow-up rules in YAML. TrueNorth checks incomplete sessions, drafts a personalised message using the LLM, and sends it via WhatsApp or any channel."
        breadcrumbs={[{ label: "Features", href: "/features" }, { label: "Async Reminders" }]}
        accentColor={ACCENT}
      />
      <div className="fp-root">
        <div className="fp-container">
          ...
          <section>
            <div style={{ marginBottom: "2rem" }}>
              <h2 className="fp-section-title">How the reminder pipeline works</h2>
              <p className="fp-section-sub">Four steps from incomplete session to personalised follow-up.</p>
            </div>
            <div className="fp-flow">
              {FLOW_STEPS.map(s => (
                <div key={s.n} className="fp-flow-step">
                  <div className="fp-flow-num" style={{ background: "rgba(255,215,0,0.08)", borderColor: "rgba(255,215,0,0.2)", color: "#FFD700" }}>{s.n}</div>
                  <div className="fp-flow-body">
                    <div className="fp-flow-title">{s.title}</div>
                    <div className="fp-flow-desc">{s.desc}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <section>
            <div style={{ marginBottom: "2rem" }}>
              <h2 className="fp-section-title">YAML configuration</h2>
              <p className="fp-section-sub">Configure reminder cadence and channel in your schema.</p>
            </div>
            <div className="fp-code-card">
              <div className="fp-code-bar">
                <span className="fp-code-filename">
              ...
              <div className="fp-code-body">
                <div><span className="fp-ky">reminders</span><span style={{color:"#263354"}}>:</span></div>
                <div><span className="fp-ky">  enabled</span><span style={{color:"#263354"}}>:</span> <span className="fp-num">true</span></div>
                <div><span className="fp-ky">  after_hours</span><span style={{color:"#263354"}}>:</span> <span className="fp-num">24</span></div>
                <div><span className="fp-ky">  max_reminders</span><span style={{color:"#263354"}}>:</span> <span className="fp-num">3</span></div>
                <div><span className="fp-ky">  channel</span><span style={{color:"#263354"}}>:</span> <span className="fp-val">whatsapp</span></div>
                <div><span className="fp-ky">  tone</span><span style={{color:"#263354"}}>:</span> <span className="fp-str">"warm and encouraging"</span></div>
                <div>&nbsp;</div>
                <div className="fp-cm"># LLM drafts message using session context</div>
                <div className="fp-cm"># → "Hi Priya! You were telling me about your fitness goals..."</div>
              </div>
            </div>
          </section>

          <div className="fp-tags">
            <span className="fp-tag hi">WhatsApp Business API</span>
            <span className="fp-tag hi">LLM-drafted</span>
            <span className="fp-tag">Custom webhooks</span>
            <span className="fp-tag">Configurable cadence</span>
            <span className="fp-tag">DPDP 2023 compliant</span>
          </div>

          <div className="fp-cta">
            <h2 className="fp-cta-title">See it in the playground</h2>
            <p className="fp-cta-sub">Start an agent, leave fields incomplete, and watch TrueNorth draft a personalised follow-up.</p>
            <div className="fp-cta-btns">
              <Link href="/playground" className="fp-btn-primary" style={{ background: ACCENT, color: "#000" }}>
                Open Playground <Arrow />
              </Link>
              <Link href="/docs/async-reminders" className="fp-btn-secondary">Read the docs</Link>
            </div>
          </div>
...
// Default export for the route — change to whichever page this file serves
export default AsyncRemindersPage;